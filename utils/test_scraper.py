...
class PropertyScraper:
    def __init__(self, base_url, output_file="properties.csv"):
        self.base_url = base_url  # must already include propertytypes & propertysubtypes
    ...
    def extract_property_type_from_link(self, link):
        """Extract property type/subtype from the URL path."""
        try:
            path = urlparse(link).path.lower()  # e.g. "/fr/detail/appartement/a-vendre/..."
            parts = path.split("/")
            for part in parts:
                if part in [
                    "maison", "villa", "immeuble-mixte", "bungalow", "fermette", "maison-de-maitre",
                    "chalet", "chateau", "appartement", "rez-de-chaussee", "penthouse", "studio",
                    "duplex", "loft", "triplex"
                ]:
                    return part
            return None
    ...
    def parse_page(self, html):
        ...
        for card in cards:
            try:
                ...
                # Property type from link
                property_type = self.extract_property_type_from_link(link)
    ...
    def test_run(self):
        """Test scraping just 2 pages."""
        self.run(max_pages=2)
...
if __name__ == "__main__":
    BASE_URL = (
        ...
        "propertysubtypes=maison,villa,immeuble-mixte,bungalow,fermette,maison-de-maitre,"
        "chalet,chateau,appartement,rez-de-chaussee,penthouse,studio,duplex,loft,triplex&page="
    )
    ...
    scraper.test_run()  # run only 2 pages for testing
